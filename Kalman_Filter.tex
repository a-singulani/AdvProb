...
The point of Theorem~2.2 is that for the model \eqref{eq:theta}--\eqref{eq:xi} this complexity collapses:
...
So Theorem~2.2 is the bridge that justifies replacing the abstract filtering problem
...
% --- Paste this right after Theorem 2.2 in your notes (or right after its statement) ---
...
Theorem~3.1 is the mathematical engine behind the Kalman update.
...
% --- Add this paragraph right after the motivation for Theorem 3.1 (or at the end of Section 3) ---

\paragraph{Relation to Theorem~2.2 (Conditional Gaussianity).}
Theorem~2.2 is the \emph{structural} statement that makes Theorem~3.1 applicable at every time step:
...
available information. More precisely, Theorem~2.2 implies that given \(\F_t^\xi\) the pair
...
That is precisely what Theorem~3.1 provides, producing the Kalman-type ``mean + gain $\times$ innovation'' update and the corresponding covariance reduction.
