...
\[
V_n(x)=\sup_{\pi}\mathbb{E}_{n,x}^{\pi}\Big[\sum_{k=n}^{N-1} r_k(X_k,f_k(X_k)) + g_N(X_N)\Big],
\]
...
\Step{1 (Split immediate and future rewards)}
By definition,
\[
...
\Step{2 (Condition on the next state)}
...
\Step{3 (Markov property / time shift)}
...
\Step{2 (Apply monotonicity of $T_n$)}
By monotonicity,
\[
v_n=T_n v_{n+1}\ge T_n V_{n+1}.
\]

\Step{3 (Bound any fixed policy)}
...
\Step{4 (Take supremum over policies)}
...
\Step{6 (Show $V_n^{\pi^*}=v_n$ by backward induction)}
...
\Step{7 (Conclude equality and optimality)}
...
\Step{4 (Lower bound via $\varepsilon$-optimal continuation)}
...
\Step{5 (Bellman equation and measurability)}
Steps 3 and 4 yield $V_n=T_nV_{n+1}$.
By $(SA_N)$-(ii), $V_n\in\mathcal{\mathbb{M}}_n$.

\Step{6 (Optimal policy from maximizers)}
...
\section{Optimal policies and stage-wise maximizers everywhere}
