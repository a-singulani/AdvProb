...
\[
(T_t f)(x) := \mathbb{E}_x\!\left[f(X_t)\right], \qquad x\in E.
\]
With this notation,
\[
\mathbb{E}_{X_s}\!\left[f(X_t)\right] = (T_t f)(X_s),
\]
...
=
\mathbb{E}_x\!\left[(T_t f)(X_s)\right].
\]
...
But by definition of $T_{t+s}$, the left-hand side of \eqref{eq:semigroup_intermediate} equals
\[
\mathbb{E}_x[f(X_{t+s})] = (T_{t+s} f)(x).
\]
By definition of $T_s$ applied to the function $T_t f$, the right-hand side equals
\[
\mathbb{E}_x[(T_t f)(X_s)] = (T_s(T_t f))(x).
\]
Therefore,
\[
(T_{t+s} f)(x) = (T_s(T_t f))(x), \qquad \forall\,x\in E,\ \forall\,f.
\]
Equivalently, as an operator identity,
\[
T_{t+s} = T_s \circ T_t, \qquad s,t\ge 0,
\]
...
\[
W_{t-H_a}:=B_{H_a+t}-B_{H_a},\qquad t\ge0,
\]
