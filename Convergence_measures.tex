...
how likely different regions $A\subset E$ are via $\mu(A)$.

\medskip

\noindent
\textbf{Why do we need a notion of convergence?}
Many limiting procedures naturally produce sequences of measures:
\begin{itemize}
...
\end{itemize}
To make sense of statements like ``$\mu_n$ converges to $\mu$'' we must specify \emph{what aspects of the mass distribution}
are required to converge.

\medskip

\noindent
\textbf{Two complementary viewpoints.}
There are two equivalent ways to compare measures.
...
\]
\end{enumerate}
The second viewpoint is often more flexible: it turns convergence of measures into convergence of real numbers,
and it interacts well with approximation arguments and functional-analytic tools.
...
\noindent
\textbf{Why do test functions determine a measure?}
A central technical point is that certain classes of functions are \emph{separating}:
knowing $\int f\,d\mu$ for all $f$ in that class uniquely determines $\mu$.
For instance, in metric spaces one can approximate indicators of closed sets by continuous bounded functions,
so $C_b(E)$ is separating. This explains why weak convergence is defined via $C_b(E)$:
it is strong enough to identify the limit measure, but weak enough to have compactness theorems.
...
Let $(E,d)$ be a metric space, and let $\B(E)$ be its Borel $\sigma$-algebra. More generally, one may start with a measurable space $(E,\Ecal)$, but in these notes we mainly use $(E,\B(E))$.

\begin{definition}[Polish space]
A metric space $(E,d)$ is called \emph{Polish} if it is complete and separable.
\end{definition}
...
\noindent
\textbf{Logical structure of the proofs.}
The separation result is established in two conceptual steps:
\begin{enumerate}[label=(\roman*)]
\item show that agreement of integrals against $C_b(E)$ implies agreement on all closed sets,
\item show that agreement on all closed sets implies equality of measures.
\end{enumerate}
The first step uses explicit continuous approximations of indicator functions of closed sets;
the second step uses measure-theoretic structure (Dynkin systems and the $\pi$--$\lambda$ theorem).
Together, they bridge the analytic world of test functions and the measure-theoretic world of sets.

\medskip

\noindent
\textbf{Why this matters later.}
Separating classes are the conceptual foundation of weak convergence,
...
\paragraph{Intuition behind Lemma~\ref{lem:closed_det}.}
This lemma formalizes the idea that closed sets already contain enough information to determine a Borel probability measure on a metric space.
...
\paragraph{Intuition behind Lemma~\ref{lem:approx_closed}.} The lemma explains why testing two probability measures against all bounded continuous functions is strong enough to recover their values on closed sets.
...
This notion is strong enough to uniquely identify the limit measure, thanks to the separating property of $C_b(E)$, yet weak enough to enjoy compactness properties such as Prokhorov’s theorem.
...
\noindent
The guiding idea is the following: two probability measures should be close if every closed set that has large mass under one measure also has almost as large mass under the other, up to a small error and a small spatial perturbation.
...
\noindent
\textbf{Intuition behind the $\varepsilon$-enlargement.}
...
The next theorem (stated without proof) clarifies the geometric and topological structure of the space of probability measures equipped with the Prokhorov metric.
...
Without these properties, compactness and subsequence arguments central to Prokhorov’s theorem would fail at a foundational level.
Thus, Theorem~4.5 shows that when the underlying space $X$ is Polish, the space $\mathcal{P}(X)$ equipped with $\beta$
...
\begin{proof}
(a)$\Leftrightarrow$(b) is just the definition.

(b)$\Rightarrow$(c): Let $(P_n)\subset\mathcal{K}$. Tightness provides compacts $K_m\uparrow$ such that
...
(c)$\Rightarrow$(a): If $\mathcal{K}$ were not tight, there would exist $\varepsilon_0>0$ such that for every compact $K$
