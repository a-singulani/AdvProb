...
\medskip
\noindent
Recall that a family of probability measures $(\Pp_n)$ on a metric space $E$ is called \emph{tight} if
for every $\eta>0$ there exists a compact set $K\subset E$ such that $\Pp_n(K)\ge 1-\eta$ for all $n$.
Tightness is the probabilistic analogue of precompactness:
it ensures that, along a subsequence, the measures $\Pp_n$ admit a weak limit.
In particular, on Polish spaces, Prokhorov's theorem shows that tightness is the key compactness
...
\]
provides a convenient way to quantify equicontinuity: small values of $w_T(\omega,\delta)$ for small $\delta$
...
starting point for functional limit theorems (in particular for Donsker's invariance principle and diffusion
approximations).
